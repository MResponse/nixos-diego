...
;; On WSL there is no compositor HiDPI scaling (WSLg's Wayland output reports
;; scale 1 and GDK_SCALE is ignored on pgtk/Wayland), so double the font to
;; match a 200% display. Desktop hosts keep the original sizes.
(let ((wsl-scale (if (string-match-p "wsl" (downcase (system-name))) 2 1)))
  (setq doom-font (font-spec :family "Iosevka" :weight 'normal :size (* 24 wsl-scale))
        doom-variable-pitch-font (font-spec :family "Iosevka" :size (* 28 wsl-scale))
        doom-big-font (font-spec :family "Iosevka" :size (* 32 wsl-scale))))
...
(when (string-match-p "wsl" (downcase (system-name)))
  ...
  (defun +wslg-fix-display ()
    "Repair the frame after WSLg display corruption (blind-pressable).
In the hard variant of microsoft/wslg#643 the Wayland configure
pipeline is wedged: Emacs-side maximize requests are silently ignored,
so first cycle the RAIL window minimize/restore from the Windows side
(bin/wslg-window-reset.ps1 via interop), then re-maximize and repaint."
    (interactive)
    (start-process "wslg-window-reset" nil
                   "/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe"
                   "-NoProfile" "-ExecutionPolicy" "Bypass" "-File"
                   "\\\\wsl.localhost\\NixOS\\home\\nixos\\nixos-config\\bin\\wslg-window-reset.ps1")
  ...
  ;; M-<f12> faengt die Sequenz ESC <f12> ab (ESC wirkt als Meta-Prefix,
  ;; wenn man beim Retten erst ESC und direkt danach F12 drueckt).
  (map! "<f12>" #'+wslg-fix-display
        "M-<f12>" #'+wslg-fix-display))
...
;; Second org root: work context (OneDrive AC_Org). May be absent on hosts
;; without OneDrive — agenda/capture wire it in only when it actually exists.
(defvar my/org-work-directory "~/org-work/"
  "Work org root (OneDrive AC_Org), parallel to the private `org-directory'.")
...
(after! org
  ...
  (setq org-agenda-files
        (append (directory-files-recursively (concat org-directory "gtd/") "\\.org$")
                ...
                ;; Work GTD (OneDrive AC_Org) — only when the work root exists here
                (let ((work-gtd (expand-file-name (concat my/org-work-directory "gtd/"))))
                  (when (file-directory-p work-gtd)
                    (directory-files-recursively work-gtd "\\.org$"))))))

;; Flycheck runs `org-lint' on every save, and `org-lint-invalid-id-link'
;; rebuilds `org-id-locations' by `file-truename'ing every agenda file.
;; With a large agenda list that costs several seconds per save. Run
;; org-lint manually via M-x org-lint when you want it.
...
;; Work capture (OneDrive AC_Org) — only when the work root exists on this host.
(after! org
  (when (file-directory-p (expand-file-name my/org-work-directory))
    (add-to-list 'org-capture-templates
                 '("w" "Work ToDo"
                   entry
                   (file+headline "~/org-work/gtd/inbox.org" "Capture")
                   "* TODO %?\n  %i\n  %a"))
    (add-to-list 'org-capture-templates
                 '("W" "Work Notes"
                   entry
                   (file+headline "~/org-work/gtd/notes.org" "Capture")
                   "* %?\n  %i\n  %a"))))
