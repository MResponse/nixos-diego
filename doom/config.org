...
* Compatibility Shims
Stubs that must be provided at config-load time, BEFORE any Doom hook,
autoload chain, or package-init can fire `(require ...)` for a missing
feature.

** render-core (emacs-reader native module)
The `emacs-reader` package's `reader.el:35` calls `(require 'render-core)`
unconditionally at top of file. The native module `render-core.so` is
not built on this host (needs `mupdf-dev` in NixOS; see
`~/.config/emacs/.local/straight/repos/reader/Makefile`). Without this
shim, the failed require propagates through `doom-first-buffer-hook` →
`global-flycheck-mode` → and blocks org-mode buffers from opening
cleanly. The reader's use-package! block is also `:disabled t` further
down for belt-and-suspenders.

This stub satisfies the require contract — PDF/EPUB rendering will
fail at call-time if a reader-mode function tries to invoke a C
function, but normal text editing (incl. Plan-0005's three-view daily
planning) doesn't need that capability.
...
** WSLg frame repaint and maximize fixes
WSLg's RDP compositor has long-standing bugs that bite pgtk Emacs on a
200% display: a freshly resized/maximized frame can be left partially
painted — content blitted at the wrong scale, the rest black — until the
next redisplay (microsoft/wslg#1058), and raising a client frame can undo
its maximization on pgtk/Wayland (Emacs bug#61710). Repaint shortly after
any frame size change and keep the server from raising frames.

Dragging/restoring the window with the mouse (especially across monitors
with different DPI) can additionally desync the Wayland buffer size from
the RAIL window — the screen then freezes on stale pixels while input
still works (microsoft/wslg#643 family). =<f12>= runs a blind-pressable
recovery: un-maximize, re-maximize, repaint.
#+begin_src emacs-lisp
...
** Emoji
Doom's =(emoji +unicode)= module pulls in emojify, whose default
=emojify-download-emojis-p ask= blocks the daemon with an invisible
y-or-n-p prompt on a fresh host (triggered by the first org/agenda
buffer — looks like a frozen Emacs). Download silently instead.
#+begin_src emacs-lisp
...
** Three-View Daily Planning (Plan-0005)
Replicates Marius's whiteboard workflow: Day View (left) + Queue View (top-right)
+ Doing View (bottom-right). Queue lists all open Dos in =~/org/gtd/today.org=
sorted by priority; Doing shows the current DOING + NEXT items with breadcrumb
ancestor path. See =shared-claude/Softwareprojekte/Nixos-Diego/plans/0005-…=.
...
** Agent Shell
Agentic coding via ACP (Agent Client Protocol). Uses claude-agent-acp subprocess with browser login — no API key needed, uses Claude Code account.
